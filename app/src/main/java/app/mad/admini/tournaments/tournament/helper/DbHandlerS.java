...
public class DbHandlerS extends SQLiteOpenHelper {
   ...
   /* public void addPlayerOdi(PlayerODIModel playermodel) {
    ...
    //calculation for ticket quantity
   /* public void getNumberOfTickets(int id,String spinner1,String spinner2,int qty1,int qty2){
        int noOfTickets = 0;
        //List<TicketModel> ticketModel = new ArrayList<>();
        ContentValues contentValues = new ContentValues();
        SQLiteDatabase db = getReadableDatabase();
        String query = "SELECT qty FROM Tickets WHERE matchID =" +id+" AND blockType = " +"'"+spinner1+"'";
        ...
        if(cursor.moveToFirst()){
           while(cursor.isAfterLast() != true){
               ...
               noOfTickets = Integer.parseInt(quantity);
           }
        }

        int newqty = noOfTickets - qty1;
        System.out.println(newqty);

        contentValues.put(QTY, newqty);
        String whereClause = TID + " =?" +"AND "+ BLOCK_TYPE + "=?";
        final String whereArgs[] = {String.valueOf(id),spinner1};

       int status = db.update(TABLE_NAME_1, contentValues, whereClause
                ,whereArgs );
        //int status = db.update(TABLE_NAME_1, contentValues, TID + "=" +id+ "AND "+BLOCK_TYPE + "= "+spinner1,
   ...
   /* public void getNumberOfTickets(int id,String spinner1,String spinner2,int qty1,int qty2){
    ...
    //insert ticket details
   /* public void updateTicketQty(TicketModel tmodel,int id,String spinner1,String spinner2,int qty1,int qty2) {
        SQLiteDatabase sqLiteDatabase = getWritableDatabase();
        ContentValues contentValues = new ContentValues();
        String qqty = String.valueOf(qty1);
       contentValues.put(TOURNAMENT_NAME, tmodel.getTournamentName());
        contentValues.put(MATCHID, tmodel.getMatchID());
        contentValues.put(STADIUM_NAME, tmodel.getStadiumName());
        contentValues.put(BLOCK_TYPE, tmodel.getBlockType());
        contentValues.put(UNIT_PRICE, tmodel.getUnitPrice());
        contentValues.put(QTY, (QTY-qqty));
        contentValues.put(STARTED, tmodel.getStared());
        contentValues.put(FINISHED, tmodel.getFinished());

        //save to table
        String whereClause = "MATCHID =" +id+" AND BLOCK_TYPE = "+spinner1;
        sqLiteDatabase.update(TABLE_NAME_1,contentValues,whereClause,null);
        //close database
        sqLiteDatabase.close();
    }*/
    @Override
